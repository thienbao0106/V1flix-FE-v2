export const handleVideo = () => {
  ...
  const miniPlayerBtn: any = document.querySelector(".mini-player-btn");
  const muteBtn: any = document.querySelector(".mute-btn");
  const captionsBtn: any = document.querySelector(".captions-btn");
  ...
  const thumbnailImg: any = document.querySelector(".thumbnail-img");
  ...
  const timelineContainer: any = document.querySelector(".timeline-container");
  ...
  function handleTimelineUpdate(e: any) {
    if (!timelineContainer || !video || !previewImg || !timelineContainer)
      return;
    const rect = timelineContainer.getBoundingClientRect();
    const percent =
      Math.min(Math.max(0, e.x - rect.x), rect.width) / rect.width;
  ...
  function toggleTheaterMode() {
    ...
    const info = document.querySelector("#info-main");
    if (!mainVideo || !mainLeftSection || !info) return;
    mainVideo.classList.toggle("md:flex");
    mainLeftSection.classList.toggle("md:w-4/6");
    info.classList.toggle("w-full");
